<x-app-layout>
    <x-slot name="title">Slide</x-slot>
    <x-slot name="header">
                    ...
                    <tbody class="divide-y-2 divide-slate-950">
                        @forelse ($slides as $slide)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4 font-black text-slate-400 text-xs">#{{ $slide->id }}</td>
                                <td class="px-6 py-4">
                                    <div class="w-24 h-16 bg-slate-100 border-2 border-slate-950 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
                                        <img src="{{ asset('storage/' . $slide->image) }}" alt="Slide Image" class="w-full h-full object-cover">
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-black text-slate-900 uppercase tracking-tight">{{ $slide->title }}</span>
                                        <span class="text-[10px] font-bold text-slate-500 mt-1 uppercase leading-tight line-clamp-2 italic">
                                            {{ $slide->desc }}
                                        </span>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">
                                        No data detected in system. Initialize content.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
