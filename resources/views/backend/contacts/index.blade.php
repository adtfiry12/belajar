<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @if($contacts->isEmpty())
            <a href="{{ route('admin.contact.create') }}" class="bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest px-6 py-2 border-2 border-slate-950 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-blue-700 transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
                + Set Contact Info
            </a>
            @endif
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="flex flex-col justify-between items-end">
                        <div class="flex gap-2">
                            <a href="{{ route('admin.contact.edit', $contact->id) }}" class="p-3 border-2 border-slate-950 bg-yellow-400 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-yellow-500 transition-all active:shadow-none active:translate-x-1 active:translate-y-1">
                            ...
                            <form action="{{ route('admin.contact.destroy', $contact->id) }}" method="POST" onsubmit="return confirm('Mau Menghapus Data, Lanjutkan?')">
                            @csrf
                            @method('DELETE')
